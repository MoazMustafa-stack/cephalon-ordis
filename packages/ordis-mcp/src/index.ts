...
server.tool("ordis_status", "Read allowance, nodes, and recent runs from the local Ordis coordinator", {}, async () => {
  const [allowance, nodes, runs] = await Promise.all(["system/allowance", "nodes", "runs"].map((path) => fetch(`${base}/api/${path}`, { headers }).then((r) => r.json())));
...
server.tool("ordis_queue", "Queue a named Ordis workflow without using a direct model API", {
  projectId: z.string().uuid(), command: z.string().min(1), arguments: z.record(z.string(), z.unknown()).optional()
}, async (input) => {
